        ...
        {mockEquipments.map((equipment) => (
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <div>
                <p className="text-xs font-bold text-blue-600 uppercase tracking-wider">{equipment.type}</p>
              ...
              <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                ...
                equipment.status === 'Error' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-600'
              }`}>
                ● {equipment.status}
              </span>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className={`font-semibold ${equipment.temperature > 40 ? 'text-red-500' : 'text-slate-700'}`}>
                  {equipment.temperature}°C
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500">전력 사용량</span>
                <span className="font-semibold text-slate-700">{equipment.powerUsage} kW</span>
              </div>
            </div>
