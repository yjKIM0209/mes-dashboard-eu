...
const TREE_DATA: EquipmentNode[] = [
  {
    id: "FAC-01", name: "창원 제1공장", type: "factory",
    children: [
      {
        id: "AREA-A", name: "A-Line (믹싱)", type: "area",
        children: [
      ...
      {
        id: "AREA-B", name: "B-Line (포장)", type: "area",
        children: [{ id: "EQP-1003", name: "충진기 #1", type: "equipment" }],
      },
    ],
  },
  {
    id: "FAC-02", name: "울산 제2공장", type: "factory",
    children: [
      {
        id: "AREA-C", name: "C-Line (검사)", type: "area",
        children: [
...
export default function EquipmentTree({ onSelectionChange }: EquipmentTreeProps) {
  const [selected, setSelected] = useState<string[]>([]);
  const [expanded, setExpanded] = useState<string[]>(() => collectExpandableIds(TREE_DATA));
  ...
  const renderTree = (nodes: EquipmentNode[]) => {
    return nodes.map((node) => {
      ...
      if (searchTerm && isEquipment && !node.name.toLowerCase().includes(searchTerm.toLowerCase())) {
        return null;
      }

      return (
        <div key={node.id} className="ml-4 border-l border-slate-200 pl-2 py-0.5">
          <div
            className={[
          ...
          >
            ...
            <span className={`text-sm ${isEquipment ? (isSelected ? "text-emerald-600 font-semibold" : "text-slate-600") : "font-bold text-slate-800"}`}>
              {node.name}
            </span>
          </div>
      ...
      <div className="flex flex-col gap-2 p-1">
        ...
        <div className="flex items-center justify-between px-1">
          <label className="flex items-center gap-2 cursor-pointer group">
            <input
              type="checkbox"
              checked={selected.length === allEquipmentIds.length && allEquipmentIds.length > 0}
              onChange={(e) => handleSelectAll(e.target.checked)}
              className="w-3.5 h-3.5 accent-emerald-600"
            />
            <span className="text-xs font-bold text-slate-600 group-hover:text-emerald-600 transition-colors">전체 설비 선택</span>
          </label>
